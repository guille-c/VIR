...
C-----------------------------------------------------------------------
      ...
      PARAMETER  (PROGNM='CBIFAKE')
      PARAMETER  (VERSN='2001 Jul 16')
C
...
C     ! number of input parameters
      PARAMETER  (NPARS=92+3*MAXS)
      PARAMETER  (NXYZ=3*MAXS)
...
C
      ...
      INTEGER BL(KMAX), UT(KMAX), T, MDATE
      ...
      REAL    THETA, U(KMAX), V(KMAX), PDATA(7), RDATA(3*MAXF)
      REAL    PASTAR, DELTPA, FWHM, SIGMA, ATTEN, RELF
      REAL    VISR(KMAX,MAXF), VISI(KMAX,MAXF), AIRMAS(KMAX), AM
      REAL    VISWT(KMAX)
      REAL    GASDEV
...
C Primary beam
      ...
      IF (BEAM.EQ.'CBI') THEN
         WRITE (*, '(1X,A)') 'Using theoretical CBI primary beam'
         CBIBM = .TRUE.
...
C-----------------------------------------------------------------------
 ...
 512     CONTINUE
      ...
      END IF
C-----------------------------------------------------------------------
C Read in the CMB image (if any).
C-----------------------------------------------------------------------
      IF (CMBDSN.NE.' ') THEN
         WRITE (*,*) 'Reading CMB sky image from ',
     :        CMBDSN(1:LEN1(CMBDSN))
         WRITE (LOGUN,*) 'Reading CMB sky image from ',
     :        CMBDSN(1:LEN1(CMBDSN))
...
C        -- File name
         WRITE (OUTDSN, '(A,I3.3,A)')
     :        PREFIX(1:LEN1(PREFIX)), IFIELD, '.uvf'
...
C
            CALL UVCOV(N, STX, STY, THETA, T, KMAX, NSAMP, UT, BL, U, V)
C
...
C           -- U(I), V(I) [in seconds]; convert to UU, VV in wavelengths
            ...
            VV = V(I)*FREQ(IF)
C           -- interpolation
            CALL GETVI2(UU, VV, VIS)
            VISR(I,IF) = VISR(I,IF) + REAL(VIS)
            VISI(I,IF) = VISI(I,IF) + AIMAG(VIS)
         END DO
      END DO
      END IF
C-----------------------------------------------------------------------
         ...
         IF (MAXFL.GT.FLXLIM) THEN
            ...
            DO I=1,NSAMP
               ...
               DO IF=1,NF
                  ...
                  VISR(I,IF) = VISR(I,IF) + FL(IF)*COS(PHASE)
                  VISI(I,IF) = VISI(I,IF) + FL(IF)*SIN(PHASE)
               END DO
            END DO
         END IF
      END DO
...
C-----------------------------------------------------------------------
      WRITE (LOGUN,*) 'Adding noise'
      RMS = NFAC*(TSYS+TATM*AIRMAS(I))
      DO I=1,NSAMP
         VISWT(I) = 1.0/(RMS*RMS)
         DO IF=1,NF
            VISR(I,IF) = VISR(I,IF) + RMS*GASDEV(ISEED)
            VISI(I,IF) = VISI(I,IF) + RMS*GASDEV(ISEED)
         END DO
      END DO
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     :            FREQ(1), BW(1), REFDAT, STOKES, STATUS)
...
C         -- Integration time (s)
          ...
          DO IF=1,NF
             RDATA(3*IF-2) = VISR(I,IF)
             RDATA(3*IF-1) = VISI(I,IF)
             RDATA(3*IF)   = VISWT(I)
          END DO
          CALL FTPPRE(UNIT3, I, 1, 3*NF, RDATA, STATUS)
       END DO
C
...
C-----------------------------------------------------------------------
 ...
 1000 CONTINUE
      ...
      SUBROUTINE UVCOV(N, X, Y, THETA, T, KMAX, K, UT, BL, U, V)
      INTEGER N, KMAX, K
...
C
 ...
 110  CONTINUE
     ...
     :                  FREQ, BW, REFDAT, STOKES, STATUS)
      INTEGER UNIT, NSAMP, NIF, STOKES, STATUS
...
C
     ...
     1            'Correlator: -1=RR, -2=LL, -3=RL, -4=LR',STATUS)
      CALL FTPKYF(UNIT, 'CRVAL3', REAL(STOKES),  1,' ',STATUS)
...
C
      CALL FTPKYS(UNIT, 'ORIGIN','CBIFAKE Caltech',' ', STATUS)
...
C
   40 GTMODL = 1
      ...
      SUBROUTINE MOSAIC(NX, NY, SPAC, RA0, DEC0, MAXF,
     :                  RA, DEC, PTX, PTY)
      INTEGER NX, NY, MAXF
      DOUBLE PRECISION SPAC, RA0, DEC0, RA(MAXF), DEC(MAXF)
      REAL PTX(MAXF), PTY(MAXF)
C
...
C
      ...
      DO IX=1,NX
         ...
         DO IY=1,NY
            ...
            PTX(I) = X
            PTY(I) = Y
            WRITE (*,'(I4,2F8.2, 2F10.4)') 
     :           I, PTX(I), PTY(I), RA(I)/RPDEG, DEC(I)/RPDEG
         END DO
      END DO
C
      END
      ...
      INTEGER STAT
C
C Read image from FITS file FILE. WHICH=1 for the CMB image (MAPARR)
C or =2 for the foreground image (FGARR).
...
C           - we already have a CMB image, so check for compatibility
     ...
     :              REFP(1),  MPRP(1), REFP(2), MPRP(2)
               CALL PUTOUT('Foreground image does not match CMB image')
               STAT = 0
               RETURN
            END IF
         ELSE
...
C              -- add the two images with appropriate scale factors
               IF (ATTEN.GT.0.0) THEN
                  IF (DOCMB) MAPWT(I) = MAPWT(I)+ATTEN*MAPARR(K)*SCALE1
      ...
      SUBROUTINE ANFFT
C-----------------------------------------------------------------------
C Do complex FFT of sky CMB image. 
...
C
      ...
      SUBROUTINE GETVI2(UU, VV, VIS)
      REAL UU, VV
      COMPLEX VIS
C-----------------------------------------------------------------------
C Return complex visibility VIS [Jy] for baseline (UU, VV) [wavelengths]
C By interpolation in array MAPFT (complex) (bilinear interpolation).
C-----------------------------------------------------------------------
      INCLUDE 'cbifake.inc'
...
C
C MAPFT has dimensions (NX/2+1, NY) with zero spatial frequency at
C (NX/2+1,  NY/2+1).
C
      X = UU/DU + ICENT
      Y = VV/DV + JCENT
     ...
     :     CALL ERROR('in GETVI2')
      V1 = GETVAL(MAPFT, ICENT, NX, NY, I,   J  )
      V2 = GETVAL(MAPFT, ICENT, NX, NY, I+1, J  )
      V3 = GETVAL(MAPFT, ICENT, NX, NY, I+1, J+1)
      V4 = GETVAL(MAPFT, ICENT, NX, NY, I,   J+1)
      VIS = (1.0-X)*(1.0-Y)*V1 +
     :           X *(1.0-Y)*V2 +
     :           X *     Y *V3 +
     :      (1.0-X)*     Y *V4
