...
C     -- character constants
     ...
     1          KET,SQUOTE,DQUOTE,QUEST,EXCL,ATSIGN,
     2          SLASH
      CHARACTER*(13)  NUMER
      PARAMETER (BLANK =' ')
      PARAMETER (EQUALS='=')
...
C
      IF ((NK.LT.N).AND.(KEYS(NK+1).EQ.BLANK8)) GOTO 202
C
...
C
  201 RESULT=KCTOR(REC,I)
     ...
     1    REC(I:I).EQ.EXCL  .OR. REC(I:I).EQ.CHAR(9) .OR.
     2    REC(I:I).EQ.SLASH) THEN
            VALUES(NK) = RESULT
            GOTO 260
      ELSE
C           write (*,*) '>>>', REC(1:I), ICHAR(REC(I:I))
            IER = 8
C                               ! bad numeric value
            GOTO 330
      END IF
