#include "reconstructor_simple.h"
...
double minimizador (Reconstructor *r, double ftol) {
  ...
  float f (float pars[]){
    ...
    imprimirLog ("        Calculando funcion %d para la %d iteracion con parametros:\n",
		 r->iter, cont_f);
    ...
    sprintf(fileout, "funcion_%d_%d_%d", r->fL->n_pols, r->iter, cont_f);
  ...
  void df (float pars[], float grad[]) {
    ...
    sprintf(fileout, "!mask_antes%d_%d.fits", r->fL->n_pols, r->iter);
    mask = do_read (r->fL->nombreFits);
    for (i = 0; i < mask->size[0]; i++) {
      for (j = 0; j < mask->size[0]; j++) {
	mask->pixels[i + j * mask->size[0]] = r->fL->mask[i][j];
    ...
    sprintf(fileout, "!MEM_antes%d_%d.fits", r->fL->n_pols, r->iter);
    ...
    imprimirLog ("\n");
    sprintf(fileout, "gradiente_%d_%d.dat", r->fL->n_pols, r->iter);
